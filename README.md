...
### v0.1.2
- 新增语速调节（-50 ~ +50）
- 新增自定义 API Key 支持（Bearer Token + `key` 参数）
- 设置面板支持折叠/展开，状态持久化
- 修复合成中背景变白的显示问题
...
应用在后端通过调用 MiloraAPI 的语音合成接口，将用户输入的文本发送至服务端进行处理，并将返回的音频数据在客户端播放或保存。

- 每个 IP 每天享有 **50 次免费调用**
- 超出免费额度后需使用 API Key，请前往 [api.milorapart.top](https://api.milorapart.top/) 获取，并在应用设置中启用并填写
